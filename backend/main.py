...
from Utils.dependency import Jwt_Token_Checker , verify_user_token
from fastapi.middleware.cors import CORSMiddleware
from Services.Docs import (add_participant , user_disconnet , get_or_create_session , empty_session)
...
class container_manger:
    ...
    async def BroadCast(self ,doc_id , message):
        for connection in self.room.get(doc_id , []):
...
async def webscoket_endpoint(websocket : WebSocket , doc_id : UUID , token : str ):
    ...
    try:
        ...
        user_disconnet(participant.id , db)
